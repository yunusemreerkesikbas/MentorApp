...
export default function PromotionForm({ initial }: { initial?: AdminPromotion | null }) {
    ...
    const usageMax = form.maxRedemptions.trim() === "" ? "sınırsız" : form.maxRedemptions;
        ...
        <div className="main-content">
            <form onSubmit={(event) => void submit(event)} aria-busy={busy}>
                <div className="row g-4">
                    ...
                    <div className="col-xl-6">
                        <FormSection title="Hedef kitle" hint={activeRule?.hint}>
                            <div className="mb-4">
                            ...
                            {form.ruleType === "NEW_USER" ? (
                                <div className="mb-4 admin-field-compact">
                            ...
                            <div className="form-check form-switch form-switch-sm ps-5">
                                ...
                                />
                                <label className="form-check-label fw-500 text-dark" htmlFor="promotion-active">
                                    {form.isActive ? "Yayında tut" : "Durduruldu"}
                                </label>
                                <div className="fs-12 text-muted">
                                    {form.isActive ? "Koşulları sağlayan ödemelerde uygulanabilir." : "Hiçbir yeni ödemede uygulanmaz."}
                                </div>
                            </div>
                        </FormSection>
                    </div>
                    ...
                    <div className="col-12">
                        <FormSection
                            title="İndirim ve limitler"
                            hint="Tutar, plan kapsamı ve kullanım tavanı. Çok dönemli indirim ödeme sağlayıcısı destekleyene kadar 1 kalmalı."
                        ...
                        >
                                ...
                                <div className="col-lg-4">
                                    ...
                                    <div className="table-responsive">
                                        <table className="table table-bordered mb-0">
                                            <tbody>
                                                <tr>
                                                    <th className="fs-10 text-dark text-uppercase">Durum</th>
                                                    <td>
                                                        <StatusBadge tone={form.isActive ? "success" : "neutral"}>
                                                            {form.isActive ? "Yayında" : "Durduruldu"}
                                                        </StatusBadge>
                                                    </td>
                                                </tr>
                                                {editing && initial ? (
                                                    <>
                                                        <tr>
                                                            <th className="fs-10 text-dark text-uppercase">Kullanım</th>
                                                            <td>{initial.redeemedCount} / {initial.maxRedemptions === null ? "sınırsız" : initial.maxRedemptions}</td>
                                                        </tr>
                                                        <tr>
                                                            <th className="fs-10 text-dark text-uppercase">Oluşturma</th>
                                                            <td>{new Date(initial.createdAt).toLocaleString("tr-TR")}</td>
                                                        </tr>
                                                    </>
                                                ) : (
                                                    <tr>
                                                        <th className="fs-10 text-dark text-uppercase">Limit</th>
                                                        <td>{form.maxRedemptionsPerUser} / kullanıcı · {usageMax} toplam</td>
                                                    </tr>
                                                )}
                                                <tr>
                                                    <th className="fs-10 text-dark text-uppercase bg-gray-100">Kayıt</th>
                                                    <td className="bg-gray-100 fw-700 text-dark">{editing ? "Güncelleme" : "Yeni kampanya"}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                            ...
                            <div className="d-flex justify-content-end gap-2 mt-3">
                                <Link href="/promotions" className="btn btn-md btn-light">Vazgeç</Link>
                                <button type="submit" className="btn btn-md btn-primary admin-submit-button" disabled={busy}>
