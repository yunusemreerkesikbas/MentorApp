...
const initial: MentorshipFollowupDto = {
  ...
  sharedDecision: "Haftayı birlikte değerlendirelim.",
  response: "PENDING",
  followUpDate: null,
  status: "OPEN",
  version: 1,
...
async function mockApi(page: Page, coach: boolean, enabled = true) {
  ...
  const user = {
    id: coach ? COACH : STUDENT,
    email: "followups@test.local",
    displayName: "Test",
    username: "test",
    avatarUrl: null,
    bio: null,
  ...
  await page.route("**/v1/**", async (route) => {
    ...
    if (path === "/v1/mentorship/followups/availability")
      return json({ enabled });
    if (path === `/v1/mentorship/students/${STUDENT}`)
      return json({
        studentId: STUDENT,
        studentDisplayName: "Ayşe",
        studentUsername: "ayse",
        acceptedAt: initial.createdAt,
        studentExamType: null,
        coachNote: null,
        riskFlags: [],
        attendedAt: null,
    ...
    if (
      path === `/v1/mentorship/students/${STUDENT}/followups` &&
      req.method() === "POST"
    ) {
      const body = req.postDataJSON();
      writes.push(body);
      const item = { ...initial, ...body, id: crypto.randomUUID() };
      items.unshift(item);
      return json(item, 201);
    }
    if (
      path.startsWith(`/v1/mentorship/students/${STUDENT}/followups/`) &&
      req.method() === "PATCH"
    ) {
      const body = req.postDataJSON();
      writes.push(body);
    ...
    if (path.endsWith("/response")) {
      ...
      const {
        ...
        privateNote: _note,
        studentId: _student,
        studentDisplayName: _name,
        replacesId: _replaces,
        ...shared
      } = items[0];
      return json(shared);
    }
    if (
      path === "/v1/mentorship/followups" ||
      path === "/v1/mentorship/my-coach/followups"
    ) {
      if (failNextRead) {
        failNextRead = false;
        return json(
          { code: "INTERNAL_ERROR", message: "Test tekrar dene" },
          500,
        );
      }
      const visible = coach
    ...
    if (path === "/v1/mentorship/my-coach")
      return json({
        linkId: "link",
...
test("coach creates, reschedules, closes and replaces a shared decision", async ({
  page,
}) => {
...
test("student only sees shared decisions and can change their response", async ({
  page,
}) => {
...
test("English error recovery and disabled feature", async ({ page }) => {
  ...
  await expect(
    page.getByRole("alert").filter({ hasText: "Test tekrar dene" }),
  ).toBeVisible();
  ...
  await page.reload();
  await expect(
    page.getByRole("heading", { name: "Our shared decisions" }),
  ).toHaveCount(0);
});
