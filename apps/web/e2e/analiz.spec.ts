...
test("boş ve ilk deneme durumlarını sakin biçimde gösterir", async ({ page }) => {
  ...
  await firstPage.goto("/tr/analiz?tab=gelisim");
  ...
  await noFocusPage.goto("/tr/analiz?tab=gelisim");
...
test("konu odağını eyleme taşır ve kanıtları klavyeyle açar", async ({ page }) => {
  ...
  await page.goto("/tr/analiz?tab=gelisim");
  ...
  await expect(coach).toHaveAttribute("href", /\/koc\/chat\?seed=.*Problemler/);
