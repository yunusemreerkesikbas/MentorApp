...
export function CoachFollowupsCard({ studentId }: { studentId: string }) {
  ...
  const load = useCallback(
    (page: number, signal: AbortSignal) =>
      fetchMentorshipFollowups(
        { studentId, view: "ALL", page, pageSize: 10 },
        signal,
      ),
    [studentId],
  );
  ...
  return (
    <Card>
      ...
      <p className="my-2 text-sm text-[var(--color-secondary)]">
        {t("followup_history_body")}
      </p>
      <div ref={actions}>
        {resource.enabled && !form && (
          <Button onClick={() => setForm({ replacesId: null })}>
            {t("followup_create")}
          </Button>
        )}
      </div>
      {resource.enabled && form && (
        <FollowupCreateForm
          key={form.replacesId ?? "new"}
          studentId={studentId}
          replacesId={form.replacesId}
          onCancel={closeForm}
          onSaved={() => {
            closeForm();
            resource.setPage(1);
        ...
        >
          ...
          {resource.data?.items.map((item) => (
            <CoachFollowupItem
              ...
              onReplace={() => setForm({ replacesId: item.id })}
            />
          ))}
          {resource.data && (
            <FollowupPagination
              {...resource.data}
              onChange={resource.setPage}
            />
          )}
        </FollowupPageState>
      </div>
