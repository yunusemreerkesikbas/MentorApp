...
export type PlanTaskSeansLinkInput = Pick<PlanTaskDto, "id" | "title" | "subject">;

export type SeansHref =
  | "/seans"
  | {
      pathname: "/seans";
...
/** Deep-link from a plan task into `/seans` with subject + task context (FE-only; no backend taskId). */
export function buildSeansHrefFromPlanTask(task: PlanTaskSeansLinkInput): SeansHref {
  ...
  return { pathname: "/seans", query };
}

export interface PlanTaskSeansContext {
