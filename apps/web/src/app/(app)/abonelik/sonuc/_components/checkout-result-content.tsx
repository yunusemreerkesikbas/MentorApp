...
export function CheckoutResultContent() {
  ...
  const cardMotion = reduceMotion
    ...
    : {
        ...
        animate: { opacity: 1, y: 0, transition: { duration: 0.35, ease: "easeOut" as const, delay: 0.06 } },
      };
  ...
  return (
    <main className="mx-auto flex w-full max-w-md flex-col gap-6 px-5 py-12 lg:px-8 lg:py-16">
      <motion.header className="text-center sm:text-left" {...headerMotion}>
        ...
        >
          {ok ? "Abonelik başladı" : "Ödeme tamamlanamadı"}
        </span>
        <h1
          className="mt-4 text-2xl font-bold"
          style={{ color: "var(--color-main)", fontFamily: "var(--font-heading)" }}
        >
          {ok ? "Hoş geldin" : "Bir sorun oluştu"}
        </h1>
      </motion.header>

      <motion.div {...cardMotion} variants={reduceMotion ? undefined : staggerItemVariants}>
        <Card>
          <SectionHeading as="h2">
            {ok ? "Premium açık" : "Tekrar deneyebilirsin"}
          </SectionHeading>
          <p className="mt-3 text-sm leading-relaxed" style={{ color: "var(--color-secondary)" }}>
            {ok
              ? "Deneme süren başladı. AI koç ve tüm Premium özellikler kullanıma hazır."
              : "Ödeme sağlayıcısından onay alamadık. Kart bilgilerini kontrol edip tekrar deneyebilirsin."}
          </p>
        </Card>
      </motion.div>
