...
const EXAM_OPTIONS: { value: ExamType; label: string; description: string }[] = [
  ...
  { value: "LGS", label: "LGS", description: "Liselere Geçiş Sistemi" },
];
...
export function ExamSettingsCard({ user }: { user: AuthUser }) {
  ...
  return (
    <Card>
      <SectionHeading
        subtitle="Resmi sınav tarihi bilgi merkezinden gelir; burada hedef sınav ailesini seçiyorsun."
      >
        Sınav türü
      </SectionHeading>
      ...
      <div
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === "ArrowDown" || e.key === "ArrowRight") {
            ...
            e.preventDefault();
            const next = EXAM_OPTIONS[(idx - 1 + EXAM_OPTIONS.length) % EXAM_OPTIONS.length];
            void handleSelect(next.value);
          }
        }}
      >
        {EXAM_OPTIONS.map((opt) => {
          ...
          return (
            <motion.button
              ...
              style={{
                border: active ? "2px solid var(--color-main)" : "1px solid #ffffff",
      ...
      {savedHint ? (
        <p className="mt-3 text-sm" style={{ color: "var(--color-secondary)" }} role="status">
          {savedHint}
        </p>
      ) : null}
    </Card>
