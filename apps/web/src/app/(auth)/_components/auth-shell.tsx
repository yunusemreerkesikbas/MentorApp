...
export function AuthShell({ children }: { children: ReactNode }) {
  ...
  const cardMotion = reduceMotion
    ...
    : {
        ...
        animate: { opacity: 1, y: 0, transition: { duration: 0.35, ease: "easeOut" as const, delay: 0.05 } },
      };
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-5 py-12">
      <motion.header className="mb-6 text-center" {...headerMotion}>
        ...
        <p className="mt-2 text-sm" style={{ color: "var(--color-secondary)" }}>
          Sınav yolunda yanında.
        </p>
      </motion.header>
      <motion.div {...cardMotion}>
        <Card className="flex flex-col gap-4">{children}</Card>
      </motion.div>
      <p className="mt-6 text-center text-xs" style={{ color: "var(--color-secondary)" }}>
        <Link href="/" className="underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2">
          Ana sayfaya dön
        </Link>
      </p>
    </main>
