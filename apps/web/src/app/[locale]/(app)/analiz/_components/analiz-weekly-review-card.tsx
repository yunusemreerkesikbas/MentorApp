...
export function AnalizWeeklyReviewCard({
  ...
  async function handleGenerate() {
    ...
    setGenerationError(null);
    try {
      const response = await http<WeeklyReviewNarrationDto>("/v1/coach/weekly-review", {
        method: "POST",
      ...
      {review.status === "INSUFFICIENT" ? (
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Link href="/analiz?tab=gir" className="flex min-h-11 flex-1 items-center justify-center rounded-[var(--radius-card)] px-4 font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus-ring)]" style={{ background: "var(--color-btn)", color: "white" }}>
              {t("enter_exam")}
            </Link>
            <Link href="/seans" className="flex min-h-11 flex-1 items-center justify-center rounded-[var(--radius-card)] px-4 font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus-ring)]" style={{ background: "var(--color-surface-muted, rgba(0,0,0,0.03))", color: "var(--color-main)" }}>
              {t("start_session")}
            </Link>
          </div>
        </div>
        ...
        <>
          <div className="grid gap-3 md:grid-cols-3">
            <ReviewSection title={t("rhythm_title")} value={t("rhythm_value", {
            ...
            <ReviewSection
              ...
              value={review.performance ? t("net_value", { net: review.performance.averageNet }) : t("no_exam")}
          ...
          {result ? (
            ...
            <Link href="/abonelik" className="flex min-h-11 items-center justify-center text-sm font-semibold underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus-ring)]" style={{ color: "var(--color-secondary)" }}>
              {t("premium_teaser")}
            </Link>
          )}
          {generationError ? (
            <div className="flex flex-col gap-2" role="alert">
              <p className="text-sm" style={{ color: "var(--color-secondary)" }}>{generationError}</p>
              <button type="button" onClick={() => void handleGenerate()} className="min-h-11 self-start underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus-ring)]">
                {t("retry")}
              </button>
            </div>
          ) : null}
...
function ReviewSection({
  ...
  return (
    <section className="flex flex-col gap-2 rounded-[var(--radius-card)] p-4" style={{ background: "var(--color-surface-muted, rgba(0,0,0,0.03))" }}>
      <h3 className="text-sm font-bold" style={{ color: "var(--color-main)" }}>{title}</h3>
      <p className="text-lg font-bold" style={{ color: "var(--color-main)" }}>{value}</p>
      <p className="text-sm leading-5" style={{ color: "var(--color-secondary)" }}>{message}</p>
    </section>
  );
}
