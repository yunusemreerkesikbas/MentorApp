...
describe("mentorship followups HTTP and database", () => {
  ...
  const draft = (extra = {}) => ({
    ...
    privateNote: "PRIVATE coach note",
    sharedDecision: "Walk through the week together",
    followUpDate: today(),
    ...extra,
  });
  ...
  beforeAll(async () => {
    ...
    for (const who of ["coach", "other", "student", "outsider"]) {
      const result = await http()
        .post("/v1/auth/signup")
        .send({
          email: `followup-${who}-${randomUUID()}@test.local`,
          password: "Sifre1234",
  ...
  afterAll(async () => {
    if (pool) {
      ...
      await sql(
        "delete from config_overrides where key in ('mentorship.enabled','mentorship.followups.enabled')",
      );
    }
    await app?.close();
    await pool?.end();
  ...
  it("requires authentication, role and an active student link", async () => {
    expect(
      (await http().get("/v1/mentorship/followups/availability")).status,
    ).toBe(401);
    expect(
      (await http().post(base()).set(auth("student")).send(draft())).status,
    ).toBe(403);
    expect(
  ...
  it("atomically deduplicates creates, rejects key reuse, and whitelists student output", async () => {
    ...
    const [first, second] = await Promise.all([
      http().post(base()).set(auth("coach")).send(input),
      http().post(base()).set(auth("coach")).send(input),
    ]);
    ...
    const privateRecord = await http()
      .post(base())
      .set(auth("coach"))
      .send(draft({ sharedDecision: null, followUpDate: null }));
  ...
  it("keeps decisions immutable and validates dates", async () => {
    expect(
      (
        await http()
          .patch(`${base()}/${record.id}`)
          .set(auth("coach"))
          .send({ version: record.version, sharedDecision: "Rewritten" })
      ).status,
    ).toBe(400);
    expect(
      (
        await http()
          .post(base())
          .set(auth("coach"))
          .send(draft({ followUpDate: "2026-02-30" }))
      ).status,
    ).toBe(400);
    expect(
      (
        await http()
          .post(base())
          .set(auth("coach"))
          .send(draft({ followUpDate: "2000-01-01" }))
      ).status,
    ).toBe(400);
  });
  it("serializes a coach edit against a student response and detects stale versions", async () => {
    const results = await Promise.all([
      http()
        .patch(`${base()}/${record.id}`)
        .set(auth("coach"))
        .send({ version: record.version, followUpDate: null }),
      http()
        .put(`${shared}/${record.id}/response`)
        .set(auth("student"))
        .send({ version: record.version, response: "ACCEPTED" }),
    ]);
    ...
    const reply = await http()
      .put(`${shared}/${record.id}/response`)
      .set(auth("student"))
      .send({ version: record.version, response: "CHANGE_REQUESTED" });
  ...
  it("orders and filters before pagination, including change requests without risk signals", async () => {
    await http()
      .post(base())
      .set(auth("coach"))
      .send(draft({ sharedDecision: null }));
    const page = await http()
      .get("/v1/mentorship/followups?view=ACTIONABLE&pageSize=1")
      .set(auth("coach"));
    expect(page.status).toBe(200);
    expect(page.body.items[0].id).toBe(record.id);
    expect(page.body.total).toBe(2);
    const next = await http()
      .get("/v1/mentorship/followups?view=ACTIONABLE&pageSize=1&page=2")
      .set(auth("coach"));
    expect(next.body.items[0].id).not.toBe(record.id);
    const outsider = await http()
      .get("/v1/mentorship/followups?view=ACTIONABLE")
      .set(auth("other"));
  ...
  it("closes independently of student consent and allows a replacement of only a closed record", async () => {
    expect(
      (
        await http()
          .post(base())
          .set(auth("coach"))
          .send(draft({ replacesId: record.id }))
      ).status,
    ).toBe(409);
    const close = await http()
      .patch(`${base()}/${record.id}`)
      .set(auth("coach"))
      .send({ version: record.version, status: "COMPLETED" });
    ...
    expect(
      (
        await http()
          .put(`${shared}/${record.id}/response`)
          .set(auth("student"))
          .send({ version: close.body.version, response: "ACCEPTED" })
      ).status,
    ).toBe(409);
    ...
    expect(
      await app
        .get(MentorshipFollowupService)
        .getNotificationTarget(record.id, "responded", record.version),
    ).toBeNull();
  });
  it("rechecks a blocked write after link end, and a re-link never opens the old period", async () => {
    ...
    await client.query("begin");
    await client.query("select set_config('app.role','SERVICE',true)");
    await client.query("select id from coach_students where id=$1 for update", [
      linkId,
    ]);
    const pending = http()
      .patch(`${base()}/${privateId}`)
      .set(auth("coach"))
      .send({ version: 1, followUpDate: today() })
      .then((result) => result);
    await client.query("update coach_students set status='ENDED' where id=$1", [
      linkId,
    ]);
    await client.query("commit");
    client.release();
    ...
    expect(
      (
        await http()
          .patch(`${base()}/${record.id}`)
          .set(auth("coach"))
          .send({ version: record.version, status: "CANCELLED" })
      ).status,
    ).toBe(404);
    expect(
      (
  ...
  it("the kill switch blocks data and erasure cascades records", async () => {
    await app
      ...
      .set(ids.coach!, "mentorship.followups.enabled", false);
    expect(
      (
        await http()
          .get("/v1/mentorship/followups/availability")
          .set(auth("coach"))
      ).body.enabled,
    ).toBe(false);
    ...
    const remaining = await sql(
      "select count(*)::int as n from mentorship_followups where link_id=$1",
      [linkId],
    );
