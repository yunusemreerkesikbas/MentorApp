  ...
  async collect(userIds: string[]): Promise<ForumAchievementEvidence[]> {
    ...
    return withServiceContext(this.db, async (tx) => {
      const [threads, posts, helpfulThreads, helpfulPosts, acceptedPosts] = await Promise.all([
        tx.select({ userId: forumThreads.authorId, earnedAt: sql<Date>`min(${forumThreads.createdAt})`.mapWith(forumThreads.createdAt) })
          .from(forumThreads)
          .where(and(inArray(forumThreads.authorId, userIds), isNull(forumThreads.deletedAt)))
          .groupBy(forumThreads.authorId),
        tx.select({ userId: forumPosts.authorId, earnedAt: sql<Date>`min(${forumPosts.createdAt})`.mapWith(forumPosts.createdAt) })
          .from(forumPosts)
          .where(and(inArray(forumPosts.authorId, userIds), isNull(forumPosts.deletedAt)))
          .groupBy(forumPosts.authorId),
        tx.select({ userId: forumThreads.authorId, earnedAt: sql<Date>`min(${forumHelpfulVotes.createdAt})`.mapWith(forumHelpfulVotes.createdAt) })
          ...
          .groupBy(forumThreads.authorId),
        tx.select({ userId: forumPosts.authorId, earnedAt: sql<Date>`min(${forumHelpfulVotes.createdAt})`.mapWith(forumHelpfulVotes.createdAt) })
          .from(forumHelpfulVotes)
