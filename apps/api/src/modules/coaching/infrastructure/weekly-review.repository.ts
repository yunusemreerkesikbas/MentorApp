...
export class WeeklyReviewRepository {
  async getEvidence(
    ...
    const [exams, subjects, photos, sessions, moods] = await Promise.all([
      tx
        .select({ id: mockExams.id, takenAt: mockExams.takenAt, totalNet: mockExams.totalNet, updatedAt: mockExams.updatedAt })
        .from(mockExams)
        .where(and(eq(mockExams.userId, userId), eq(mockExams.examId, examId), gte(mockExams.takenAt, previousStart), lt(mockExams.takenAt, currentEnd))),
      tx
        .select({
        ...
        .where(and(eq(mockExams.userId, userId), eq(mockExams.examId, examId), gte(mockExams.takenAt, previousStart), lt(mockExams.takenAt, currentEnd))),
      tx
        .select({ subjectRef: mockExamPhotoCategorizations.subjectRef, createdAt: mockExamPhotoCategorizations.createdAt })
      ...
      tx
        ...
        .where(and(
          ...
          gte(studySessions.endedAt, currentStart),
          lt(studySessions.endedAt, currentEnd),
        )),
      tx
        .select({ checkinDate: moodCheckins.checkinDate, mood: moodCheckins.mood, updatedAt: moodCheckins.updatedAt })
        .from(moodCheckins)
        .where(and(
    ...
    return { exams, subjects, photos, sessions, moods };
  }
}
