...
describe("coach mock-exam context lifecycle", () => {
  it("keeps context pending after failure and consumes it only after success is recorded", () => {
    expect(coach.resolvePendingMockExamContext(MOCK_EXAM_ID, null)).toBe(MOCK_EXAM_ID);
    expect(coach.resolvePendingMockExamContext(MOCK_EXAM_ID, null)).toBe(MOCK_EXAM_ID);
    expect(coach.resolvePendingMockExamContext(MOCK_EXAM_ID, MOCK_EXAM_ID)).toBeUndefined();
  });

  it("removes only contextMockExamId after success", () => {
    expect(
      coach.removeMockExamContextFromUrl(
        `http://localhost:3000/tr/koc/chat?seed=review&contextMockExamId=${MOCK_EXAM_ID}&c=thread#composer`,
      ),
    ).toBe("/tr/koc/chat?seed=review&c=thread#composer");
  });
