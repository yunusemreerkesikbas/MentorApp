...
| **W8 Mentorship (koç)** | 🟡 | **Phase-2 pull-forward** ([workstreams](./workstreams.md)). Human coach↔student link (rotating invite code + double opt-in), risk-sorted roster with a rule-based cohort band + seat counter + coach-side data-scope mirror, single-student report, weekly assignment composer with topic + coach note and reusable saved programs, program feedback loop (deleted/completed assignments travel back), append-only dropped-assignment log, one standing note per link, daily risk digest cron, **coach attention mark** (roster + digest read one rule, so a student the coach already handled stops repeating), **coach application + admin vetting queue** (approving records the verdict and grants COACH; the evidence is a structured claim plus the admin's mark of what they checked, no credential upload), **the vetted profile on the student's consent screen** with the coach editing their own two lines behind the repo's first Tier-1 contact detector. Coverage: **216 API unit+e2e · 44 browser (2 viewports)**. **Coach intelligence layer (APP-085-087):** cohort brief on panel entry (rules stay the floor — the chips beside each line are `risk-flags.ts`, the "new" badge is a set comparison, only the sentence is a model's), AI-drafted homework loaded into the composer (nothing written until the coach submits), and the student's mirror of what is travelling to their coach. Behind `mentorship.enabled` (default off) and `mentorship.risk_digest.enabled` (default off); neither has been flipped in production ([mentorship](../features/mentorship.md)) | Phase 2/3: in-app chat · `apps/panel` move · Tier-2 auto-moderation (AI smart brief, seat billing, the vetting queue, the coach profile and the intelligence layer shipped: APP-076-087) |
...
- **CI is RED, and it is release gate #1 (2026-09-07).** `security-release-checklist.md`'s first
  gate is "Tam CI … yeşil". APP-084 got the pipeline from *nothing ever ran* to *everything runs
  except the last step*; unit/integration is fully green (API 2202, web 562). The remaining red is
  `Analiz browser smoke`: **39 unique Playwright failures across 11 spec files, both viewports**,
  reproduced on every recent master run. Largest cluster `knowledge.spec.ts` (12) — the public SEO
  surface, which no feature doc mentions. This blocks the `forum.enabled` / `mentorship.enabled`
  flips more directly than any missing feature does.
- **The exam calendar seed has expired, and the exam-anchored half of the product is dark
  (2026-09-07).** `exams.seed.json` carries three `EXAM_DATE` events, all KPSS, all in July 2026;
  YKS and LGS carry none. `calendar.util.ts` filters `examDate >= today`, so
  `getExamCalendarByFamily` returns **null for every family**: no countdown, no mock-exam entry
  (`<NoExamSeed />`), dead subject pickers on plan and session, no subject/topic tagging in the
  notebook, and no weekly recap. Not a bug — the code fails safely and silently, which is exactly
  why two months passed unnoticed. Fixing it needs real ÖSYM dates (guardrail §4 #1 forbids
  generating them) plus something that makes the next expiry visible.
