...
| **W4b — Promotions** | Kupon/kampanya/otomatik indirim motoru + redemption defteri. Payments ona bağımlı, tersi değil; kural sinyalleri parametreyle gelir (başka context'in tablosu okunmaz). | `modules/promotions/**` · `admin/(general)/promotions/**` · `web: paywall indirim yüzeyi (Faz 2)` |
