...
| `GET /v1/coach/plan-adaptation/brief`                | Premium wizard seed: coverage line, verified evidence, suggested defaults (no model call) |
...
- **2026-09-24 · Coach marker stays internal.** The stage-one QA run found that the streaming
  ...
  before; there is no API change. Regression coverage is in the coach unit and E2E suites.
  Related: `apps/api/src/modules/ai/application/chat.service.ts`,
  `apps/api/test/ai-coach.e2e-spec.ts`, `docs/qa/2026-09-24-security-performance.md`.

- **2026-09-24 · Gün seçici öğrencinin en çok çalıştığı günlerle açılır.** Brief önerisine `weekdays` eklendi (ISO, 1 = Pazartesi, yoksa null). Son 28 günün tamamlanmış seansları İstanbul takvimine göre hafta günlerine ayrılır (`coachRhythm` içindeki `extract(isodow …)` sorgusu, havuzda `weekdayActivity28d`). `suggestPlanBrief` ritmin gün sayısı (`days`) kadar en aktif hafta gününü seçer; eşitlikte odak dakikası kazanır ve öğrencinin hiç çalışmadığı bir gün seçilmez. Sihirbaz bu günleri işaretli açar, satırlarda "Ritmin" yazar, üstte "Son 4 haftada en çok bu günlerde çalıştın." görünür. Öğrencinin seçimi her zaman kazanır. **Kök düzeltme (master'da da vardı):** `coachRhythm().lastActiveAt` ham `max(started_at)` ile okunduğu için Date değil string geliyordu. `getCoachRhythm` `toISOString()` çağırırken patlıyor, `CoachEvidenceService.safe()` bunu yutuyordu. Sonuç olarak 28 günde seansı olan her öğrencide RECENT_RHYTHM ve LONG_TERM_RHYTHM kanıtları ile ritim önerileri (gün, dakika) hiç oluşmuyordu. Düzeltme `.mapWith(studySessions.startedAt)` ile yapıldı. **Kullanım:** Plan → Koçla planla. **Gotcha:** aynı ham `sql<Date>` kalıbı forum ve başarım sorgularında da var, ayrı görev olarak işaretlendi. Birim testleri bu hatayı yakalayamazdı çünkü mock'lar gerçek Date döndürüyor. Kapsamı `test/coach-evidence.e2e-spec.ts` gerçek DB testi sağlar; bu test UTC pazar 22:30 seansının pazartesi sayıldığını da doğrular. İlgili: `study-session.repository.ts`, `session.service.ts`, `coach-evidence.ts`, `plan-adaptation.ts`, `plan-coach-adaptation-brief*.tsx`.
- **2026-09-24 · Koçla planla: gün seçici, serbest dakika, sınırsız ders, nottaki boş günler.** Öğrenci "çarşamba ve cuma gününü boş bırak" yazdı, plan tam o iki güne ders koydu. İki kök neden vardı. Modele pencere yalnız ISO tarihlerle gidiyordu, hangi tarihin çarşamba olduğunu bilmiyordu. Eksik günleri dolduran `fillStudyDays` da notu hiç görmüyordu, çarşambayı "en boş gün" diye seçti. Düzeltme ref→reason desenini izler: prompt her pencere gününü adıyla verir ("2026-09-24 Perşembe"). Model nottaki boş günleri `offDates` olarak döner. Backend bu tarihlere ADD ya da MOVE koymaz, dolgu da onları atlar. Sihirbazın "Kaç gün?" adımı "Hangi günler?" oldu: bugünden başlayan 7 satır, `studyWeekdays` (ISO, 1 = Pazartesi) gönderir. Sunucu bunları kendi penceresindeki tarihlere çevirir (`studyDatesFor`) ve yalnız o günlere yazar. `days` ile `studyWeekdays` birlikte gelirse doğrulama reddeder. `minutesPerDay` artık 10–600 arası her tam sayıyı alır; kartlar kısayol, altta "Ya da kendin yaz" alanı var. 3 ders sınırı kalktı (API'de yalnız 30 adlık kötüye kullanım tavanı). Kullanılmayan `composePlanAdaptationNote` ve metin anahtarları silindi. Eval'e gerçek istekle `plan-adaptation-note-free-days-tr` vakası eklendi; gpt-4o-mini iki koşuda da boş günleri doğru döndü. **Kullanım:** Plan → Koçla planla. **Gotcha:** gün seçici tarayıcının bugününü, API sunucunun UTC bugününü kullanır. İstanbul'da 00:00–03:00 arası pencereler bir gün kayar (`date.util.ts`'teki bilinen UTC notu). Model notu hiç anlamazsa `offDates` boş gelir ve nottaki istek eskisi gibi korunmaz; seçicideki günler ise her zaman bağlayıcıdır. İlgili: `plan-adaptation.ts`, `plan-adaptation.service.ts`, `packages/validation/src/ai.ts`, `plan-coach-adaptation-brief*.ts*`, `e2e/plan.spec.ts`, `test/eval/openai-prompt.eval.ts`.
- **2026-09-23 · Düşük modda boş sayfa yok.** Paneldeki "Bugünü biraz hafifletmek ister misin? Koçla planla" kartı artık yalnız bugün bekleyen görev varken çıkar (`GreetingRow` `canLighten`, panelin `today.tasks` verisinden). Mood uyarlaması yalnız bugünün bekleyen görevlerini hafifletir, hafifletilecek görev yokken kart öğrenciyi çıkmaza götürüyordu. Kural gereği `NO_CHANGE` yine dönerse (ör. görev arada tamamlandıysa) sayfa artık boş değil: model çağrısı olmadan backend doğrulanmış bir cümle yazar ("Bugünkü 1 görevini tamamlamışsın. Bugün dinlenmek de planın parçası." ya da görev yoksa "Bugün seni bekleyen bir görev yok…") ve MOOD kanıtlarını `usedEvidence` olarak döner. Üst mesaj da bu duruma özel: `status` API sözleşmesi için `NO_CHANGE` kalır, metin `coaching.planAdaptation.REST` anahtarından gelir ("Bugün hafifletilecek bir görev kalmadı. Yarın kaldığın yerden birlikte devam ederiz."). Genel `NO_CHANGE` metni ("küçük adımlarla devam") dinlenme cümlesiyle çelişiyordu. İlgili: `grounding-fact.ts` (`REST`), `plan-adaptation.service.ts`, `i18n/locales/{tr,en}/coaching.json`, `greeting-row.tsx`, `panel-shell.tsx`, `e2e/panel.spec.ts`.

- **2026-09-23 · Görev kartı garantisi deterministik katmanda.** `CREATE_PLAN_TASK` izni olan bir turda model `<<TASK>>` işaretini atlarsa (eval'de gpt-4o-mini üç vakada atladı) `ChatService.proposedTask` kartı kendisi kurar. Dersi yalnız koçun görünür yanıtında geçen taksonomi derslerinden seçer: önce doğrulanmış odak/zayıf ders, yoksa yanıtta ilk anılan ders. Yanıt hiçbir ders anmıyorsa "25 dk kısa başlangıç". Hafıza ve tahmin dersi seçemez, bu yüzden kart üstündeki cümleyle çelişmez; eski hafızada Matematik olsa da "önceliğim Tarih" diyen öğrenci Tarih kartı görür. Modelin kendi işareti varsa o kullanılır. Deneme değerlendirmesi ve izinsiz turlar kart almaz. Blocking, stream ve regenerate aynı yolu kullanır. Eval'in `required-action` kontrolü `model-task-marker` review'üne döndü ve detayda öğrencinin göreceği yedek kartı yazar. **Gotcha:** blok süresi sabit 25 dk (varsayılan seans); ritme göre ayarlamak istenirse `fallbackCoachTask` tek yer. İlgili: `suggested-task.ts` (`fallbackCoachTask`), `chat.service.ts`, `test/eval/openai-prompt.eval.ts`.

- **2026-09-23 · Prompt eval yeniden çalışır ve havuz vakalarını kapsar.** `test:eval:openai` 27 Temmuz'dan beri modül yüklenirken düşüyordu: haftalık anlatım fixture'ı `recap`/`plan`/`highlights` içermiyordu. Vizyon notu da eski pozisyonel imzayla çağrılıyordu ve `undefined` hedefli bir prompt üretiyordu. İkisi de güncel şekle getirildi. Üç vaka eklendi (19 vaka): son 30 günde zengin kanıtlı plan uyarlaması (katı ref ve tarih kontrolü), seçili denemeli değerlendirme turu, zayıf dersli "ne çalışayım?". **gpt-4o-mini (üretim) sonucu:** 19 vakada 4 hard hata, maliyet ~0,003 $. Plan uyarlaması ve değerlendirme hard kontrollerden geçiyor; plan 4 değişikliğin 4'ünde doğrulanmış kanıta atıf yaptı. Hatalardan 3'ü TASK işaretinin eksik olması (iki eski vaka ve yeni "ne çalışayım?"), 1'i eski anksiyete vakasında rakam arayan regex (model "üç görevin birini" yazdı). **Gotcha:** varsayılan `OPENAI_EVAL_MODEL=gpt-5` ile her çağrı `incomplete` biter. `AI_MAX_OUTPUT_TOKENS=600` akıl yürütme token'larına yetmiyor (kısa bir promptta 543/600, 448'i akıl yürütme). Adaptör bunu sağlayıcı hatası sayar; gpt-5 geçişinden önce tavan ya da akıl yürütme düzeyi kararı gerekir. Kullanım: `OPENAI_EVAL_MODEL=gpt-4o-mini pnpm --filter @mentor/api test:eval:openai`. İlgili: `test/eval/openai-prompt.eval.ts`, `ai.constants.ts`, `openai-llm.adapter.ts`.

- **2026-09-23 · Koçla planla ortak havuzdan beslenir ve her öneriyi gerekçelendirir.** `PlanAdaptationService` artık `ContextBuilder` yerine `CoachEvidenceService` okur. PLAN isteği en fazla 6 kanıt satırı alır (`E1..En`: sınav evresi, zayıf dersler, defter konuları, geçen haftanın ders dağılımı ve plan takibi, ritim, mood, hedef), izinli hafıza ve koçluk tercihi de girer. MOOD ve SESSION dar kalır: 3 satır, hafıza yok. Model her değişiklik için bir `evidenceRef` seçer; backend onu doğrulanmış cümleye çevirip `changes[].reason` olarak döner, bilinmeyen ref gerekçesiz kalır. Ders seçilmediyse boş günler zayıf derslerle dolar; seçilen derse taşınan blok "Bu hafta ağırlık vermek istediğin ders." der. `groundingLine` kapsam cümlesidir: "4 denemene, 9 yanlış kartına ve son 28 gündeki 12 seansına baktık." Yeni `GET /v1/coach/plan-adaptation/brief` (premium kapısı, model/kota/bütçe yok) sihirbaza kapsam cümlesi, kanıt ve önerilen gün/dk/ders döner. **Sınav evresi kararı:** 2026-07-20'de koç prompt'larından çıkarılan geri sayım yalnız kaba evre olarak (90+ / 30-90 / son 30 gün) ve yalnız plan uyarlamasında geri döndü. Satırı backend yazar, modele tarih ya da gün sayısı gitmez; son 30 günde prompt tekrar ve soru çözümü ister. Sohbet evreyi hiç görmez. **Web:** sihirbaz Puhu'nun kapsam cümlesiyle açılır; gün ve dakika "Ritmin", dersler "Koçun önerisi" ile ön-seçili gelir (öğrencinin seçimi her zaman kazanır, brief hatası bugünkü varsayılanları bırakır). Önizlemede PREMIUM rozetli, katlanır "Koçun baktıkları" ve satır başı "Neden: …" var. **Gotcha:** web uygula isteğine `reason`'lı satırları geri yollar; şema alanı atar (e2e ile doğrulandı). Sahte adaptör kanıt varsa `E1` iliştirir. Model davranışı değiştiği için yayından önce `test:eval:openai`. İlgili: `plan-adaptation.ts`, `plan-adaptation.service.ts`, `grounding-fact.ts`, `ai-chat.controller.ts`, `plan-coach-adaptation-{action,brief,brief-steps,preview}.tsx`, `coach-evidence-list.tsx`, `test/coach-evidence.e2e-spec.ts`, `e2e/plan.spec.ts`.

- **2026-09-23 · Koçla değerlendir performans turu olarak başlar.** Seçili deneme varsa (`contextMockExamId`, regenerate'te kayıtlı bağlam) `ChatService` turu `PERFORMANCE` olarak planlar; anahtar kelime sınıflandırması ve ilk tanışma kalibrasyonu atlanır. Önceden konu seed'i ("…yanlış defterimde tekrar ediyor…") TR ve EN'de GENERAL'a düşüyordu; kalibre olmamış premium öğrenci değerlendirme yerine kalibrasyon sorusu alıyordu. PERFORMANCE kanıtı artık deneme, zayıf dersler ve defter konularıdır; PLAN niyetine zayıf dersler (ikinci sırada) ve geçen haftanın ders dağılımı eklendi. `EXAM_PHASE` hiçbir sohbet listesinde yok (§4 #1). Değerlendirme bağlamı odak trendini, en çok tekrar eden üç konuyu ve gelişim döngüsü adımlarını da taşır; prompt döngünün eksik adımını önermesini ister. **Web:** premium öğrencide "AI koçla değerlendir" sohbeti kendiliğinden başlatır; coin ve free öğrencide mesaj yalnız doldurulur, gönderim onun elindedir. **Gotcha:** sohbet prompt'u değişti, yayından önce `test:eval:openai`. İlgili: `coach-turn-planner.ts`, `chat.service.ts`, `analysis-coach-prompt.ts`, `coach-chat-shell.tsx`, `e2e/coach.spec.ts`.
...
- **2026-09-18 — Notebook prelabel is unused on web.** Adding a mistake no longer asks
  ...
  for now (quota + §4 #2 classify-not-solve still apply if something calls it). Related:
  `notebook-add-panel.tsx`, `lib/notebook.ts`.
