...
| `coaching` | `/v1/coaching/today`, `/v1/coaching/analysis` *(+ `photoSubjectSignals`)*, `/v1/plan-tasks`, `/v1/study-sessions`, `/v1/mock-exams`, `/v1/coaching/mood-checkins` | daily loop + deneme analysis (server-computed net, personal trend) + rule-based mood | MVP | ✅ |
| `ai` | `/v1/coach/access` · `/v1/coach/chat` · `/v1/coach/mood-reflection` *(premium AI-adaptive mood reply; cached per day)* · `/v1/coach/photo-access` · `/v1/mock-exams/photo-upload-url` · `/v1/mock-exams/{id}/categorize-photo` *(premium vision; `ai.photo.monthly_limit`)* · `/v1/admin/ai/reembed` *(SUPER_ADMIN)* _(planned: multi-turn)_ | AI coach chat with **RAG grounding** + **coin→AI spend** + **photo→subject categorize** (Gemini/fake vision, fake/R2 storage) + **mood AI-adaptive reflection + mood grounding**; premium flat + rate limits | MVP | 🟡 |
