...
| `admin` | `/v1/admin/{users…,audit-log,config…,users/:id/economy…,content/articles…}` *(ADMIN; content also EDITOR)* _(planned: refund, metrics)_ | user mgmt + KVKK, append-only audit log, config registry + feature flags, economy adjust, **content editor (article CRUD + publish)**; refund/metrics pending | MVP | 🟡 |
