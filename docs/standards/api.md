...
| `forum` | `/v1/forum/zones` *(list/create)* · `/v1/forum/zones/:slug` · `/v1/forum/zones/:id/{owner,join,members,members/:userId/approve}` *(flag `forum.enabled`)* | Zone primitive (announcement/chat/qa) + scoped membership; curated staff creation, external OWNER, OPEN/REQUEST join + owner/staff approval, two-plane authz (`forum.policy`). Threads/QA/search/reactions/report→moderation + verification/coin/C-layer = later slices / Phase 2 | MVP (slice 1) | 🟡 |
