...
| `ai` | `/v1/coach/chat` *(PremiumGuard; `ai.enabled`; rate-limit)* · `/v1/admin/ai/reembed` *(SUPER_ADMIN)* _(planned: multi-turn)_ | premium AI coach chat with **RAG grounding** (pgvector over info_articles; answer + `sources[]`; dates→data card) via LlmPort fake/OpenAI; PII-free context; usage meter §7 | MVP | 🟡 |
