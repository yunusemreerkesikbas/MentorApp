...
| `ai` | `/v1/coach/chat` *(PremiumGuard; `ai.enabled` flag; daily rate-limit)* _(planned: RAG grounding + source links, multi-turn)_ | premium AI coach chat (single-turn, refusal-grounded via LlmPort fake/OpenAI; PII-free context; usage meter §7) | MVP | 🟡 |
