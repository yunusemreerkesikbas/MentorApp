...
| `forum`         | Existing zone/thread/post/moderation API + `/v1/forum/hub` · `/v1/forum/feed?scope=relevant\|following&sort=trending\|recent\|top` · `/v1/forum/trends?scope=relevant\|exam\|general&limit=` · `/v1/forum/tags` · `/v1/forum/zones/:slug/feed` · `/v1/forum/search` _(threads/questions/zones/tags/public-safe people, max 5 each; legacy QA fields additive)_ · `PATCH /v1/forum/{threads\|posts}/:id` · `PUT/DELETE /v1/forum/{threads\|posts}/:id/helpful-vote` · `GET /v1/forum/{threads\|posts}/:id/reactions?page=&pageSize=&emoji=` _(visibility-gated, paginated public reactor identities; optional emoji filter)_ · `/v1/admin/forum/{tags,featured-thread}` _(EDITOR, audited; featured GET/PUT returns `ForumFeaturedAdminView` with selected `thread` summary)_ · legacy `/v1/forum/feed/following` remains compatible _(flag `forum.enabled`)_ | Zone/membership/thread/post/moderation retained. Discovery V2 adds curated max-3 tags, positive QA helpful votes, time+interaction locked owner edits, featured fallback, N+1-free server ranking and versioned opaque cursors. Trends aggregate active tagged threads in the configured window and only return public, open zones relevant to the viewer's exam. Search returns only public/open zones, separates QA questions, and consumes identity public service without reading identity tables/PII. Reaction identity lists are resolved only after target visibility checks. Tier-1 verification/coin/C-layer/presence = later | MVP + Discovery V2 | 🟡 |
