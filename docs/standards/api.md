...
| `ai` | `/v1/coach/access` · `/v1/coach/chat` *(premium flat OR earned coin; `ai.enabled`; rate-limits)* · `/v1/admin/ai/reembed` *(SUPER_ADMIN)* _(planned: multi-turn)_ | AI coach chat with **RAG grounding** + **coin→AI spend** (free path); premium flat + `ai.chat.daily_limit`; free coin path + `ai.chat.free_coin_daily_limit`; access probe for web gate | MVP | 🟡 |
