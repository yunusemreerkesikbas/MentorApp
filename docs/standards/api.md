...
| `forum` | `/v1/forum/zones` *(list/create)* · `/v1/forum/zones/:slug` · `/v1/forum/zones/:id/{owner,join,members,members/:userId/approve,threads}` · `/v1/forum/threads/:threadId` *(delete)* · `/v1/forum/threads/:threadId/{pin,reactions}` *(flag `forum.enabled`)* | Zone primitive + scoped membership (slice 1). **Slice 2:** flat thread feed (cursor) + reactions + pin for CHAT/ANNOUNCEMENT — post requires ACTIVE membership (ANNOUNCEMENT = owner/mod/staff only); soft-delete by author/mod; `@Throttle` on post. QA/search/report→moderation + verification/coin/C-layer = later slices / Phase 2 | MVP (slice 1+2) | 🟡 |
