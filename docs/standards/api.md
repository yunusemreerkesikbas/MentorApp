...
| `economy` | `/v1/economy/{balance,ledger,invite,invite/redeem,quests}` *(self, flag-gated; quests v2 returns category/period/reward/action metadata)* · `/v1/admin/users/:id/economy{,/adjust}` *(ADMIN; overview incl. quests)* | append-only XP/Coin ledger (balance=sum), capped reward engine, **invite→conversion→coin**, **onboarding + daily ritual quests (auto-grant)**, **coin spend→AI chat** (via EconomyService.spend); admin manual adjust; milestone quests pending | MVP (light) | 🟡 |
