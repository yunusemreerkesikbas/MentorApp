...
| `economy` | `/v1/economy/{balance,ledger,invite,invite/redeem,quests}` *(self, flag-gated)* · `/v1/admin/users/:id/economy{,/adjust}` *(ADMIN; overview incl. quests)* | append-only XP/Coin ledger (balance=sum), capped reward engine, **invite→conversion→coin**, **onboarding quests (auto-grant)**, **coin spend→AI chat** (via EconomyService.spend); admin manual adjust; habit/milestone quests pending | MVP (light) | 🟡 |
| `forum` | `/v1/forum/zones` *(list/create)* · `/v1/forum/zones/:slug` · `/v1/forum/zones/:id/{owner,join,members,members/:userId/approve,threads}` · `/v1/forum/threads/:threadId` *(get detail/delete)* · `/v1/forum/threads/:threadId/{pin,reactions,answers,accept/:postId}` · `/v1/forum/answers/:postId` *(delete)* · `/v1/forum/search` *(flag `forum.enabled`)* | Zone + membership (slice 1). **Slice 2:** flat feed + reactions + pin (CHAT/ANNOUNCEMENT). **Slice 3:** QA — questions (titled threads) + answers (`forum_posts`) + asker-only one-shot accept → `forum.answer.accepted` event → economy grants XP (`forum.xp.accepted_answer`); Postgres full-text search over QA questions. report→moderation + verification/coin/C-layer = later slices / Phase 2 | MVP (slice 1–3) | 🟡 |
| `community` | folds into `forum` (mahalle = closed/auto-assign zone variant) | neighborhood, presence/leaderboard, live room | Phase 2 | ⛔ |
