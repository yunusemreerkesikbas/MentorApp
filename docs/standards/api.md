...
| `ai` | `/v1/coach/access` · `/v1/coach/chat` · `/v1/coach/photo-access` · `/v1/mock-exams/photo-upload-url` · `/v1/mock-exams/{id}/categorize-photo` *(premium vision; `ai.photo.monthly_limit`)* · `/v1/admin/ai/reembed` *(SUPER_ADMIN)* _(planned: multi-turn)_ | AI coach chat with **RAG grounding** + **coin→AI spend** + **photo→subject categorize** (Gemini/fake vision, fake/R2 storage); premium flat + rate limits | MVP | 🟡 |
